...
- 10 个始终 active 的 Block，`neuron_size=100`、`hold_tick=4`、`ticktime=250 ms`。Block 0 是 Eye ReadIn，Block 1 是 Hand ReadOut，2–9 是普通 Core。实验仅构造 EyeAdapter 和 27 离散量、零连续量的 HandAdapter。
...
新一轮正式运行会把配置与 source SHA256 写入独立的 `runs/stage_zero_local/potential_goodness/config.json`，不覆盖此前 strict-reward 的[锁定文件](../reports/stage_zero_local_plasticity_lock.json)、[预检](../reports/stage_zero_local_plasticity_preflight.md)或[报告](../reports/stage_zero_local_plasticity_report.md)。活动实现与正式命令见[实验 README](../experiments/stage_zero/README.md)。本轮仅修改协议与测试，尚未运行新的正式实验。
