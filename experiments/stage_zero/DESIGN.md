...
The phase budget and seed set come from the run command line and are recorded in the run-local config; this pilot uses seeds 11 and 22 with 135 initial, 540 training and 135 frozen episodes per seed. Targets and delay values are balanced per phase. Each letter alternates red and blue appearances; the phase-specific first color is drawn independently. Target, color, delay, and Hand RNG streams have distinct fixed seeds documented in the run-local config.

Each run writes its frozen configuration and source hashes to `config.json` in the selected output directory. Use `runs/stage_zero_local/tau_scan/` for the pilot so the earlier strict-reward raw data is preserved. The old lock, preflight and formal report in `reports/` document that earlier run only. Raw JSONL and sparse checkpoints remain ignored by Git; the audit regenerates stimulus hashes and verifies every row.

The success classification is conservative: parameter changes and nonzero local states alone do not support visual→action learning. Multiple seeds, positive target tendency, reduced non-target tendency, frozen retention and actual exact success improvement are needed for SUPPORTED. Delay differences are observational; this experiment cannot infer a required change to the formal time definition. The pilot runs below the formal scale, so its report lists trend indicators only and applies no classification.
