...
from .environment import ACTIONS, VisualEnvironment, balanced_targets, exact_goodness
...
def run_episode(model, environment, target, *, episode, phase_episode, phase,
    ...
    goodness = exact_goodness(target, signal.a)
    ...
    row = {
        ...
        "correct_exact_match": int(goodness == 1.0),
            ...
            for index, target in enumerate(balanced_targets(count, sequence_seed)):
                ...
                if (index + 1) % 27 == 0 or index + 1 == count:
                    print(json.dumps({"seed": seed, "phase": phase, "episodes": index + 1,
                                      "total": count, "successes": sum(r["goodness"] for r in phase_rows),
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="ACNT Stage Zero strict delayed scalar experiment")
